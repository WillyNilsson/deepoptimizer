...
IMPORTANT: To ensure high-quality analysis, limit your response to the 10 MOST CRITICAL issues found.
If there are more than 10 issues, focus on the most important ones (errors first, then warnings).

Return a JSON array of issues found. Start your response with <json> and end with </json>.
...
[
  {
    "severity": "error",
    "category": "bug",
    "title": "Memory leak from accumulating loss tensors",
    "description": "The code accumulates loss tensors which keeps the computation graph in memory",
    "line_numbers": [17],
    "suggestion": "Use loss.item() to extract scalar value: total_loss += loss.item()",
    "confidence": 0.95
  }
]
</json>
...
Prioritize issues by severity (errors > warnings > info) and impact on the codebase."""
